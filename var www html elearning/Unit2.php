...
if (isset($_POST['lessoneml'])){
    $_SESSION['lessoneml2'] = $_POST['lessoneml'];
    return;
}

if (isset($_POST['quizeml'])){
    $_SESSION['quizeml2'] = $_POST['quizeml'];
    return;
}

$xml = simplexml_load_string($_SESSION['lessoneml2']);
$intro = $xml->attributes()->{'Intro'};
foreach ($xml->Part as $Part) {
    array_push($summaryPart, array("summarypart" => $Part->Summary));
...
switch ($prop_id) { // parses xml data from array into html document
    ...
    case 1:
        ...
        <a class=active href="Unit2.php?page=1">1</a>
        <a href="Unit2.php?page=2">2</a>
        <a href="Unit2.php?page=3">3</a>
        <a href="Unit2.php?page=4">Quiz</a>
        </div>';
        for ($x = 0; $x < 3; $x++) { // iterates the required lessons in part 1 of the Unit
            $body.='<p style="margin: 20px;">' . $lessonUnit[$x]["info"];
            $body.='<br><br><b>Markup:</b><br><br><img src=' . $lessonUnit[$x]["markup"] .'></p>'; // MIGHT NEED QUOTES HERE FOR FILEPATH
        }
        $summary = $summaryPart[0]["summarypart"];
        ChromePhp::log($summary);
        break;
    case 2:
        $pag = '<div class="pagination">
        <a href="Unit2.php?page=1">1</a>
        <a class=active href="Unit2.php?page=2">2</a>
        <a href="Unit2.php?page=3">3</a>
        <a href="Unit2.php?page=4">Quiz</a>
    </div>';
        ...
        $summary = $summaryPart[1]["summarypart"];
    ...
    case 3:
        ...
        <a href="Unit2.php?page=1">1</a>
        <a href="Unit2.php?page=2">2</a>
        <a class=active href="Unit2.php?page=3">3</a>
        <a href="Unit2.php?page=4">Quiz</a>
    </div>';
        for ($x = 6; $x < 9; $x++) { // iterates the required lessons in part 3 of the Unit
            $body.='<p style="margin: 20px;">' . $lessonUnit[$x]["info"];
            $body.='<br><br><b>Markup:</b><br><br><img src=' . $lessonUnit[$x]["markup"] .'></p>'; // MIGHT NEED QUOTES HERE FOR FILEPATH
        }
        $summary = $summaryPart[2]["summarypart"];
    ...
    case 4:
        ...
        <a href="Unit2.php?page=2">2</a>
        <a href="Unit2.php?page=3">3</a>
        <a class=active href="Unit2.php?page=4">Quiz</a>
        </div>';


        $xml = simplexml_load_string($_SESSION['quizeml2']);
        $intro = $xml->attributes()->{'Intro'};
...
$script = /** @lang JavaScript */ <<< HEREDOC
...
$template->assign('title', 'Lesson 2: Intro to Client-side Scripting in JavaScript'); // assign content values
